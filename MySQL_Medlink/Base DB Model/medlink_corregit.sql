...
-- Schema medlink
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `medlink` DEFAULT CHARACTER SET utf8 ;
USE `medlink` ;
...
CREATE TABLE IF NOT EXISTS `medlink`.`person` (
  ...
  `pers_last_name_2` VARCHAR(50) NULL,
...
CREATE TABLE IF NOT EXISTS `medlink`.`patient` (
  `pati_person_id` INT NOT NULL,
  ...
  `pati_remarks` MEDIUMTEXT NULL,
  `pati_caregiver_id` INT NULL,
  PRIMARY KEY (`pati_person_id`),
  INDEX `fk_patient_person_idx` (`pati_person_id` ASC),
  UNIQUE INDEX `pati_person_id_UNIQUE` (`pati_person_id` ASC),
...
CREATE TABLE IF NOT EXISTS `medlink`.`specialty` (
  `spec_id` INT NOT NULL AUTO_INCREMENT,
  `spec_name` VARCHAR(60) NOT NULL,
  PRIMARY KEY (`spec_id`),
  UNIQUE INDEX `spec_name_UNIQUE` (`spec_name` ASC),
  UNIQUE INDEX `spec_id_UNIQUE` (`spec_id` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `medlink`.`doctor` (
  `doct_person_id` INT NOT NULL,
  ...
  UNIQUE INDEX `doct_person_id_UNIQUE` (`doct_person_id` ASC),
...
CREATE TABLE IF NOT EXISTS `medlink`.`treatment` (
  `trea_id` INT NOT NULL AUTO_INCREMENT,
  ...
  `trea_observations` MEDIUMTEXT NULL,
  ...
  PRIMARY KEY (`trea_id`),
  UNIQUE INDEX `trea_id_UNIQUE` (`trea_id` ASC),
...
CREATE TABLE IF NOT EXISTS `medlink`.`medicine_category` (
  `meca_id` INT NOT NULL AUTO_INCREMENT,
  `meca_name` VARCHAR(60) NOT NULL,
  `meca_description` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`meca_id`),
  UNIQUE INDEX `meca_id_UNIQUE` (`meca_id` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `medlink`.`medicine` (
  `medi_id` INT NOT NULL AUTO_INCREMENT,
  `medi_name` VARCHAR(50) NOT NULL,
  `medi_unit_of_measure` INT CHECK (`medi_unit_of_measure` IS NOT NULL AND `medi_unit_of_measure` IN (0,1)),
  `medi_category_id` INT NOT NULL,
  PRIMARY KEY (`medi_id`),
  UNIQUE INDEX `medi_id_UNIQUE` (`medi_id` ASC),
...
CREATE TABLE IF NOT EXISTS `medlink`.`treatment_medicine` (
  `trme_treatment_id` INT NOT NULL,
  `trme_medicine_id` INT NOT NULL,
  ...
  UNIQUE INDEX `trme_treatment_id_UNIQUE` (`trme_treatment_id` ASC),
